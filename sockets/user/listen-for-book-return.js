...
module.exports = (socket, users, books) => {
    socket.on("listen-for-book-return", async (data) => {
        ...
        if(book.exists){
            poll = bookRef.onSnapshot((doc) => {
                if(doc.data().available){
                    // Book has been considered returned.
                    // Alert student.
                    socket.emit("listen-for-book-return-result", { message: "Book turned in!", bgColor: "#55FF55", txColor: "#000000" });
        ...
        setTimeout(() => {
            ...
            if(!snapshotDetached && poll != undefined){
                poll();
                snapshotDetached = true;
                socket.emit("listen-for-book-return-result", { message: "Listener disconnected: updates paused.", bgColor: "#FF5555", txColor: "#FFFFFF" });
            }
        }, 60000);
